    ...
    for number in greaterList:
      if (number in smallerList):
        if (index in KeyValues["Matches"].keys()):
          ...
          # print("match:", excelNames[index], "with", gNames["first"][number], gNames["last"][number], number,smallerList, greaterList, index  )
          KeyValues["Matches"][index] = [number]

      elif (index in KeyValues["Partial Matches"].keys()):
        KeyValues["Partial Matches"][index].append(number)
      else:
        # print("partial match:", excelNames[index], "with", gNames["first"][number], gNames["last"][number], number, index, smallerList, greaterList )
